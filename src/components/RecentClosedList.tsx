    ...
    <Card className="animate-fade-up p-4" style={delayMs ? { animationDelay: `${delayMs}ms` } : undefined}>
      ...
      <ul className="relative mt-2.5 flex flex-col">
        ...
        {deals.map((r, i) => (
          <li
            key={r.id}
            className="animate-fade-up relative flex items-center gap-3 border-s-2 border-s-transparent px-1.5 py-2 transition-colors hover:border-s-[var(--status-good)] hover:bg-[var(--surface-2)]/60"
            style={{ animationDelay: `${(delayMs ?? 0) + 120 + i * 45}ms` }}
          >
            <span className="relative z-10 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[var(--surface)] text-[var(--status-good)] ring-4 ring-[var(--surface)]">
              <span className="absolute inset-0 rounded-full bg-[color-mix(in_oklab,var(--status-good)_14%,transparent)]" aria-hidden="true" />
